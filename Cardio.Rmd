...
---
title: 'Homework #4: Machine Learning Part II'
date: "Due December 3, 2021 by 11:59pm EST"
output: html_document
---
...
#### Question 1
**Using the `lda` function, fit an LDA model to the training set that predicts `DEATH_EVENT` based on all of the other variables except `time`. Then, use the `predict` function to get probability estimates for the test set. Your code should look something like this to get the predicted probabilities: `predict(lda_fit, newdata = test_set)$posterior[,2]`. Next, convert the probabilities to predicted response labels (use a cutoff of 0.5). Obtain the accuracy, confusion matrix, and other relevant diagnostic statistics for the test set. Explain what the confusion matrix and the sensitivity/specificity are telling you about the types of errors that your model is making.**
...
#### Question 2
Using the `qda` function, fit an QDA model to the training set that predicts `DEATH_EVENT` based on all of the other variables except `time`. Then, use the `predict` function to get probability estimates for the test set. Your code should look something like this to get the predicted probabilities: `predict(qda_fit, newdata = test_set)$posterior[,2]`. Next, convert the probabilities to predicted response labels (use a cutoff of 0.5). Obtain the accuracy, confusion matrix, and other relevant diagnostic statistics for the test set. Explain what the confusion matrix and the sensitivity/specificity are telling you about the types of errors that your model is making.
...
#### Question 3
**Using the `rpart` function, fit a decision tree model to the training set that predicts `DEATH_EVENT` based on all of the other variables except `time`. Then, use the `predict` function to get probability estimates for the test set, and convert the probabilities to predicted response labels (use a cutoff of 0.5). Obtain the accuracy, confusion matrix, and other relevant diagnostic statistics for the test set. Explain what the confusion matrix and the sensitivity/specificity are telling you about the types of errors that your model is making.**
...
#### Question 4
**Using the `randomForest` function, fit a Random Forest model to the training set that predicts `DEATH_EVENT` based on all of the other variables except `time`. Then, use the `predict` function to get probability estimates for the test set, and convert the probabilities to predicted response labels (use a cutoff of 0.5). Obtain the accuracy, confusion matrix, and other relevant diagnostic statistics for the test set. Explain what the confusion matrix and the sensitivity/specificity are telling you about the types of errors that your model is making.**
...
#### Question 5
**In the paper the authors find that `ejection_fraction` and `serum_creatinine` were the most predictive of the outcome. Calculate variable importance using the Gini index for the Random Forest model from question 4. Order the features from highest Gini index to smallest, i.e., in descending order. Do your top 2 most predictive features (features with the highest Gini index) match those of the authors?**
...
#### Question 6
**Rebuild the same 4 models you used for questions 1-4 (LDA, QDA, decision tree, Random Forest), but with `ejection_fraction` and `serum_creatinine` as the only predictors. Obtain the accuracy, confusion matrix, and other relevant diagnostic statistics for the test set. Is there an increase in overall accuracy for each model? Which model performs the best in terms of overall accuracy?**
...
#### Question 7
**Using the models you built in question 6, plot 4 ROC curves - one for each model - on the same graph. Be sure to include a legend and appropriate axis labels.** 
...
#### Question 8
**Calculate the AUC for each of the 4 models from question 6. Which one achieves the highest AUC? Which model would you choose as the best model? Note: There is no one right answer for this - as long as you justify your choice you will receive credit.** 
